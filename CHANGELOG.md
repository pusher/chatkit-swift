...
- Resolved a bug which could results in a crash when setting a new cursor for a room.
...
- Resolved a concurrency bug which could cause app crash when accessing a user store
  from multiple threads. This tended to manifest while creating lots of subscriptions
  concurrently, including when returning from the background.
...
- Add `onMessageDeleted` hook that is invoked when a single message is deleted.
- Move api version to v5

## [1.5.3](https://github.com/pusher/chatkit-swift/compare/1.5.2...1.5.3) - 2019-06-24
