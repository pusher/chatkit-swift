...
final class PCMultipartBasicMessageEnricher {
    public let userStore: PCGlobalUserStore
    public let room: PCRoom
    let logger: PPLogger
    
    fileprivate var completionOrderList: [Int] = []
    fileprivate var messageIDToCompletionHandlers: [Int: (PCMultipartMessage?, Error?) -> Void] = [:]
    fileprivate var enrichedMessagesAwaitingCompletionCalls: [Int: PCMultipartMessageEnrichmentResult] = [:]
    fileprivate let messageEnrichmentQueue = DispatchQueue(label: "com.pusher.chatkit.multiPartMessage-enrichment-\(UUID().uuidString)")
    
    fileprivate var userIDsBeingRetrieved: [String] = []
    fileprivate var userIDsToBasicMessageIDs: [String: [Int]] = [:]
    fileprivate var messagesAwaitingEnrichmentDependentOnUserRetrieval: [Int: PCBasicMultipartMessage] = [:]
    ...
    func enrich(_ basicMessage: PCBasicMultipartMessage, completionHandler: @escaping (PCMultipartMessage?, Error?) -> Void) {
        ...
        userRetrievalQueue.async(flags: .barrier) {
            ...
            self.userStore.user(id: basicMessage.senderID) { [weak self] user, err in
                ...
                strongSelf.userRetrievalQueue.async(flags: .barrier) {
                    ...
                    let basicMessages = basicMessageIDs.compactMap { bmID -> PCBasicMultipartMessage? in
    ...
    fileprivate func enrichMessagesWithUser(_ user: PCUser, messages: [PCBasicMultipartMessage]) {
        messages.forEach { basicMessage in
            let message = PCMultipartMessage(
                id: basicMessage.id,
                sender: user,
                room: self.room,
    ...
    fileprivate func callCompletionHandlersForEnrichedMessagesWithIDsLessThanOrEqualTo(id: Int, result: PCMultipartMessageEnrichmentResult) {
...
public enum PCMultipartMessageEnrichmentResult {
    case success(PCMultipartMessage)
    case error(Error)
}
