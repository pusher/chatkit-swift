...
class EncodableTests: XCTestCase {
    ...
    func test_encode_dictionaryAllSupportedValueTypes_noProblem() {
        ...
        let when = {
            try JSONEncoder.default.encode(encodable)
        }
        ...
        XCTAssertNoThrow(try when()) { data in
            ...
            XCTAssertString(dataAsString, contains: "\"null-dict\":{\"null-key\":null}")
            XCTAssertString(dataAsString, contains: "\"null-array\":[null]")
            XCTAssertString(dataAsString, contains: "\"empty-dict\":{}")
            XCTAssertString(dataAsString, contains: "\"empty-array\":[]")
        }
    }
